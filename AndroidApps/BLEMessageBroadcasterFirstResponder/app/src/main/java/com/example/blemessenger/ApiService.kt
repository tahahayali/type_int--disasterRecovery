...
object ApiService {
    ...
    suspend fun updateProfile(
        ...
        try {
            val url = URL("$BASE_URL/api/update")
            val connection = url.openConnection() as HttpURLConnection
            ...
            Log.d(TAG, "Sending update request: $jsonPayload")
            ...
            if (responseCode == HttpURLConnection.HTTP_OK) {
                ...
                Log.d(TAG, "Update successful: $response")
                Result.success(response)
            } else {
                val errorResponse = try {
                    BufferedReader(InputStreamReader(connection.errorStream)).use {
                ...
                Log.e(TAG, "Update failed: $errorResponse")
                Result.failure(Exception("Update failed: $errorResponse"))
            }
